...
class Generator:
    ...
    def generate_tests(
        ...
        for i in range(count):
            br.generate_test(
                target_dir.joinpath(f"test_{i}.c"), total_blocks, blocks_cap, vars_cap
            )
    ...
    def add_sub_parser(self, sub_parser) -> ArgumentParser:
        self.analyze_parser: ArgumentParser = sub_parser.add_parser(
            "generate", prog="generate"
        )
        self.analyze_parser.add_argument(
            "--out_dir", default="tests", help="Path to output folder"
        )
        self.analyze_parser.add_argument(
            "--repeats", type=int, default=1, help="Count of repeats to generated test"
        )
        self.analyze_parser.add_argument(
