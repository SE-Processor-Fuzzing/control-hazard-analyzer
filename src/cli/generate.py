...
class Generate(Utility):
    ...
    def generate_tests(self, target_dir: Path, count: int, max_depth: int = 6, base_seed: int | None = None) -> None:
        ...
        :param base_seed: Seed on the basis of which seeds for tests are generated. Base-seed the base seed also serve as a seed for the first test
        """
        ...
        if base_seed is None:
            ...
            print(f"Provided seed is not an integer. No test generated. Exiting...")
            self.logger.warn(f"Provided seed is not an integer. No test generated. Exiting...")
            return
        rd.seed(base_seed)
    ...
    def run(self) -> None:
        ...
        then create directories for tests, generate seeds for every test based on provided seed and start generating tests
        """
